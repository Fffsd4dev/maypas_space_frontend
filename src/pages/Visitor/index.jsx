...
const SeatBookingSystem = () => {
  ...
  const triggerPaystackPopup = (access_code) => {
    ...
    popup.resumeTransaction(access_code, {
      onSuccess: (response) => {
        handleConfirmBooking(response.reference);
  ...
  const handleBookingSubmit = async (e) => {
    ...
    try {
      ...
      let hasEmptyFields = false;
        if (visitorToken) { 
         hasEmptyFields = bookingFormData.chosen_days.some(day => 
      ...
      const url = visitorToken
        ? `${import.meta.env.VITE_BACKEND_URL}/api/${visitorSlug}/user/initiate/pay`
   ...
   const handleConfirmBooking = async (reference) => {
    ...
    try {
      ...
      let hasEmptyFieldsConfirm = false;
        if (visitorToken) { 
         hasEmptyFieldsConfirm = bookingFormData.chosen_days.some(day => 
          !day.day || !day.start_time || !day.end_time
        );
      } else {
        hasEmptyFieldsConfirm = bookingFormData.chosen_days.some(day => 
      ...
      const url = visitorToken
        ? `${import.meta.env.VITE_BACKEND_URL}/api/${visitorSlug}/user/confirm/pay`
        : `${import.meta.env.VITE_BACKEND_URL}/api/${visitorSlug}/confirm/pay/spot`;
    ...
    if (!response.ok) {
      ...
      const result = await response.json();
      toast.success(result.message || " Payment made and Space booked successfully!");
      handleBookingClose();
      // Show success popup
    showPaystackPopup(
