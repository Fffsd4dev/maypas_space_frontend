...
const VatRegistrationModal = ({ showVat, onHide, myVat, onSubmit }) => {
...
//   const [selectedLocation, setSelectedLocation] = useState(null);
  ...
  useEffect(() => {
    if (myVat) {
      ...
      setFormData({
        ...
        // location_id: myVat.location_id || "",
        // bank_name: myVat.bank_name||"",
        // account_number: myVat.account_number || "",
        // account_name: myVat.account_name || "",
        key: myVat.key || "",
      });
    } else {
      setFormData({
  ...
  useEffect(() => {
    if (showVat && user?.tenantToken) {
      fetchLocations();
    }
  }, [showVat, user?.tenantToken]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = myVat
        ? `${import.meta.env.VITE_BACKEND_URL}/api/${tenantSlug}/saveVatkey`
        : `${import.meta.env.VITE_BACKEND_URL}/api/${tenantSlug}/saveVatkey`;

      const method = myVat ? "POST" : "POST";
  ...
  return (
    <Modal show={showVat} onHide={onHide} centered>
      <Modal.Header className="bg-light" closeButton>
        <Modal.Title>
          {myVat ? "Vat" : "Add Your Vat"}
        </Modal.Title>
      </Modal.Header>
...
export default VatRegistrationModal;