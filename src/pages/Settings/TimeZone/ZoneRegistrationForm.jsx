...
const ZoneRegistrationModal = ({ show, onHide, timeZone, locations = [], onSubmit }) => {
  const { user } = useAuthContext();
  ...
  // Reset form when timeZone changes
  useEffect(() => {
    if (timeZone) {
      setFormData({
        location_id: timeZone.location_id || "",
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    try {
      ...
      const url = timeZone
        ? `${import.meta.env.VITE_BACKEND_URL}/api/${tenantSlug}/update/time/zone/${timeZone.id}`
        : `${import.meta.env.VITE_BACKEND_URL}/api/${tenantSlug}/create/time/zone`;
      ...
      const payload = {
        location_id: formData.location_id,
        utc_time_zone: selectedTz ? selectedTz.utc_time_zone : "",
        timezone_name: formData.timezone, // Store the actual timezone name
      };
  ...
  }, [tenantToken, tenantSlug, timeZone, formData, onSubmit, onHide, validateForm]);

  return (
    <Modal show={show} onHide={onHide} centered size="lg" backdrop="static">
      <Modal.Header className="bg-light" closeButton>
        <Modal.Title>
          {timeZone ? "Edit Time Zone" : "Add Time Zone"}
        </Modal.Title>
      </Modal.Header>
      <Modal.Body className="p-4">
        <Form onSubmit={handleSubmit}>
          <Form.Group className="mb-3" controlId="location_id">
            ...
            <Form.Select
              ...
              onChange={handleInputChange}
              required
              disabled={isLoading || timeZone} // Disable when editing
            >
              <option value="">Select a location</option>
              {locations.length > 0 ? (
                locations.map((location) => (
            ...
            {isLoading ? (
              <>
                <Spinner
                  ...
                  className="me-2"
                />
                {timeZone ? "Updating..." : "Adding..."}
              </>
            ) : (
              <>{timeZone ? "Update" : "Add"} Time Zone</>
            )}
          </Button>
        </Form>
      </Modal.Body>
