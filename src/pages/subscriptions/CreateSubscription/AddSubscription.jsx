...
const AddSubscription = ({
  ...
  const schemaResolver = yupResolver(yup.object().shape({
    ...
    duration: yup.string().required("Please enter duration (in months)").matches(/^\d{10}$/, "Phone number is not valid"),
    location: yup.string().required("Please enter location")
  }));
  return <>
      <Modal show={show} onHide={onHide} aria-labelledby="contained-modal-title-vcenter" centered>
        <Modal.Header className="bg-light" onHide={onHide} closeButton>
          <Modal.Title className="m-0">Add New Subscription</Modal.Title>
        </Modal.Header>
        <Modal.Body className="p-4">
          <VerticalForm onSubmit={onSubmit} resolver={schemaResolver}>
            ...
            <FormInput label="Duration (in months)" type="text" name="duration" placeholder="Enter duration in months" containerClass={"mb-3"} />
