...
const Invoices = () => {
  ...
  const getSpacePayment = (invoice) => {
    if (!invoice?.space_payment?.[0]) {
      return {
        amount: 0,
        payment_status: "N/A",
    ...
    return invoice.space_payment[0];
  };
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (Array.isArray(result.invoices)) {
        // Safely sort the data by checking for space_payment
  ...
  const handleCloseInvoiceButton = (myInvoiceID, myInvoiceRef) => {
    const spacePayment = getSpacePayment(data.find(inv => inv.id === myInvoiceID));
    
    // Only allow closing if status is pending
    if (spacePayment.payment_status === "pending") {
      setCloseInvoicePopup({
        isVisible: true,
        myInvoiceID,
        myInvoiceRef,
  ...
  const columns = [
    ...
    },
    {
      ...
      accessor: (row) => {
        ...
        const symbol = currencySymbols[row.location_id] || "$";
        return `${symbol} ${spacePayment.amount || 0}`;
      },
      sort: true,
    },
    {
