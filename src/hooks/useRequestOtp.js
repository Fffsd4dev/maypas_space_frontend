...
const useRequestOtp = () => {
  ...
  const redirectUser = () => {
    ...
    if (redirectLink) navigate(redirectLink); else navigate(`/${tenantSlug}/auth/confirmNewPassword`);
  };
  ...
  const requestOtp = handleSubmit(async (data) => {
    ...
    console.log(data);
    try {
      const res = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/${tenantSlug}/confirm-user`, {
        method: 'POST',
        headers: {
